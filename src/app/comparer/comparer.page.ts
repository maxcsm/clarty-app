...
@Component({
  ...
  templateUrl: './comparer.page.html',
  styleUrls: ['./comparer.page.scss'],
})
export class ComparerPage implements OnInit {
...
uploadFileToOpenAi(fileBlob: Blob) {
  ...
  async comparer() {
    ...
    this.redditService.addPost("CompareOffre",data)  
    .subscribe(async (response) => {
     console.log(response); 
  
     this.comparaisonData=response;
  ...
  async getMyOffre(){
    ...
    this.redditService.getDataBypageByUser(this.iduser,this.page,"list_offres",this.per_page,this.order_id,this.order_by,this.category,this.status,this.filter).subscribe(data => {
   setTimeout(() => { 
      loader.dismiss();
     }, 200);
    console.log(data);
